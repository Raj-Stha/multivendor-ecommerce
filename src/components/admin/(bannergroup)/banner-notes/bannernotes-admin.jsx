...
import BannerNoteList from "./bannernote-list";
import AddBannerNotesForm from "./form/AddBannerNotesForm";
import { SidebarTrigger } from "@/components/ui/sidebar";

export default function PromoNotesAdmin({
  initialData,
  initialMeta,
  initialPage,
}) {
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              <DialogTrigger asChild>
                <Button className="flex items-center gap-2 bg-primary text-white px-4 py-4 hover:bg-primary hover:opacity-90 cursor-pointer transition">
                  ...
                  <span className="hidden md:inline">Add Banner Notes</span>
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-[500px] max-h-[80%] overflow-y-auto rounded-lg shadow-lg">
                <DialogHeader>
                  <DialogTitle>Add New Banner Notes</DialogTitle>
                </DialogHeader>
                <AddBannerNotesForm setIsOpen={setIsOpen} />
              </DialogContent>
            </Dialog>
          </div>
        </div>
