import EditManufacturerNotesForm from "./form/EditManufacturerNotesForm";
import { Button } from "@/components/ui/button";

export default function ManufacturerNoteList({ data, page, setPage, meta }) {
  return (
    <div>
      {data && data.length > 0 ? (
        <>
          {data.map((d) => (
            ...
            >
              <div className="flex items-center justify-between w-full p-4">
                ...
                <EditManufacturerNotesForm data={d} />
              </div>
            </div>
          ))}
