    ...
    async (searchTerm) => {
      ...
      try {
        ...
        const products = data.details.map((product) => ({
          ...
          sellPrice: product.variants[0]?.product_price - (product.variants[0]?.product_discount || 0),
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-96 overflow-hidden">
          {error ? (
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-gray-600 text-sm font-medium mt-2">Searching...</p>
            </div>
          ) : results.length > 0 ? (
            <>
              {totalResults > 0 && (
                    ...
                    <div className="flex-shrink-0 text-right">
                      <div className="flex flex-col items-end">
                        {product.sellPrice < product.price ? (
                          <>
                            <span className="text-sm font-semibold text-primary">{formatPrice(product.sellPrice)}</span>
