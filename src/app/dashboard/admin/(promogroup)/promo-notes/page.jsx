...
import PromoNoteList from "../../../../../components/admin/(promogroup)/promo-notes/PromoNoteList";

import AddPromoNotesForm from "../../../../../components/admin/(promogroup)/promo-notes/form/AddPromoNotesForm";
...
export default function PromoNotes() {
  ...
  const getAllPromoNotes = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to fetch promo");
      }
    } catch (error) {
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              <Button className="flex items-center gap-2 bg-primary text-white px-4 py-4  hover:bg-primary hover:opacity-90 cursor-pointer transition">
                ...
                <span className="hidden md:inline">Add Promo Notes</span>
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[500px] max-h-[80%] overflow-y-auto rounded-lg shadow-lg">
              <DialogHeader>
                <DialogTitle>Add New Promo Notes</DialogTitle>
              </DialogHeader>
              <AddPromoNotesForm setIsOpen={setIsOpen} />
            </DialogContent>
          </Dialog>
        </div>
      </div>
